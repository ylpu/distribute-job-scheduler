# Introduction
thales是一款自主研发的分布式任务调度系统，系统支持command,shell,hive,spark,python,http,sql,flink,clickhouse等各种类型任务类型，实测每天可调度百万级别任务，满足市场上90%以上公司调度需求。
...
* 依次执行如下脚本
  * scheduler-api/scheduler-controller/bin/api-server.sh start ,启动成功后可以通过http://localhost:8085/swagger-ui.html 查看接口文档
