...
public class SQLExecutor extends AbstractCommonExecutor {
    ...
    private void select(Connection conn, JdbcExtractor extractor, String sql, Map<String, Object> map,
            String logOutPath) throws Exception {
        if (conn != null) {
        ...
        if (conn != null) {
            ...
            try {
                ...
                try {
                    ...
                    FileUtils.writeFile(
                            "failed to execute task " + request.getId() + " with exception " + e1.getMessage(),
                            logOutPath);
                    throw e1;
                }
            }
