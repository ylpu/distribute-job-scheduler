        ...
        if(servers != null && servers.size() > 0){
            ...
            if(sortedServers != null && sortedServers.size() > 0) {
                if(lastFailedHosts == null || lastFailedHosts.length == 0) {
                    ...
                    List<WorkerResponse> runningServers = sortedServers.stream().filter(
                            hostInfo -> !Arrays.asList(lastFailedHosts).contains(hostInfo.getHost()))
                            .collect(Collectors.toList());
                    if(runningServers != null && runningServers.size() > 0) {
                        WorkerResponse worker = runningServers.get(0);
                        if(worker.getMemoryUsage() > 95) {
                            throw new RuntimeException("worker " + worker.getHost() +  " 内存使用率大于95%，为 " + worker.getMemoryUsage());
                        }
