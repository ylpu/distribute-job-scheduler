...
public class ConnectionController {
    ...
    public SchedulerResponse<PageInfo<ConnectionResponse>> paging(
            @RequestParam(value = "pageNo", required = false, defaultValue = "1") Integer pageNo,
            @RequestParam(value = "pageSize", required = false, defaultValue = "10") Integer pageSize,
            @RequestParam(value = "connectionId", required = false) String connectionId) {
        return new SchedulerResponse<PageInfo<ConnectionResponse>>(
                connectionService.findAll(connectionId, pageNo, pageSize));
    }
}
